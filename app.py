...
def set_status(mac,status):
    with get_db_connection() as conn:
        with conn.cursor() as cur:
            if status=="ACTIVE":
                cur.execute("UPDATE clients SET ativo=TRUE, expiration_timestamp=NULL WHERE mac=%s",(mac,))
            else:
                cur.execute("UPDATE clients SET ativo=FALSE WHERE mac=%s",(mac,))
            conn.commit()
...
def rename(mac):
    ...
    exp_ts = None
    if date_str:
        try: exp_ts = datetime.fromisoformat(date_str)
        except: pass
    elif secs_str:
        try: exp_ts = datetime.utcnow() + timedelta(seconds=int(secs_str))
        except: pass
